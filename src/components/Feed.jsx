...
function PostCard({ post, profile, myLikedSet, onToggleLike }) {
  ...
  async function handleAddComment() {
    ...
    setPosting(true)
    const { error } = await supabase.from('post_comments').insert({ post_id: post.id, author_id: profile.id, author_name: profile.name, content: newComment.trim() })
    setPosting(false)
    if (!error) { setNewComment(''); setCommentCount((c) => c + 1); loadComments() }
  }
    ...
    <div className="card" style={{ padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <div>
          ...
          {post.author_specialization && <span className="badge badge-gold" style={{ marginLeft: 6 }}>{post.author_specialization}</span>}
        </div>
      ...
      {post.image_url && (
        <img src={post.image_url} alt="" style={{ width: '100%', borderRadius: 10, marginBottom: 10, border: '1px solid var(--ring)', display: 'block' }} />
      )}
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--ring)' }}>
        <button onClick={handleLike} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: liked ? 'var(--gold-bright)' : 'var(--text-soft)', fontWeight: liked ? 700 : 500, display: 'flex', alignItems: 'center', gap: 5, padding: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
            {comments.map((c) => (
              <div key={c.id} style={{ fontSize: 13 }}>
                <span style={{ fontWeight: 600 }}>{c.author_name}</span> <span style={{ color: 'var(--text-soft)' }}>{c.content}</span>
              </div>
            ))}
            {commentsLoaded && comments.length === 0 && <p style={{ fontSize: 12, color: 'var(--text-soft)' }}>No comments yet. Be the first.</p>}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Add a comment..." onKeyDown={(e) => { if (e.key === 'Enter') handleAddComment() }} />
            <button onClick={handleAddComment} disabled={posting} className="btn-gold" style={{ padding: '8px 14px', borderRadius: 8, fontSize: 13, whiteSpace: 'nowrap' }}>Send</button>
          </div>
        </div>
      )}
    </div>
...
export default function Feed({ profile }) {
  ...
  async function load() {
    let query = supabase.from('posts').select('*, post_likes(count), post_comments(count)').order('created_at', { ascending: false }).limit(50)
    ...
    if (profile.role === 'ps') {
      ...
      const idsToShow = Array.from(new Set([...followedIds, profile.id]))
      query = query.in('author_id', idsToShow)
    }

    const { data: postRows } = await query
    const shaped = (postRows || []).map((p) => ({ ...p, likeCount: p.post_likes?.[0]?.count || 0, commentCount: p.post_comments?.[0]?.count || 0 }))
    setPosts(shaped)
  ...
  useEffect(() => {
    load()
    const channel = supabase.channel('feed-changes').on('postgres_changes', { event: '*', schema: 'public', table: 'posts' }, () => load()).subscribe()
  ...
  async function handlePost() {
    ...
    try {
      ...
      const { error } = await supabase.from('posts').insert({ author_id: profile.id, author_name: profile.name, author_specialization: profile.specialization, content: content.trim(), image_url: imageUrl })
        ...
        <div className="card" style={{ padding: 16, marginBottom: 20 }}>
          ...
          {imagePreview && (
            <div style={{ position: 'relative', marginBottom: 10 }}>
              ...
              <button onClick={clearImage} type="button" style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.7)', color: '#fff', border: 'none', borderRadius: 6, width: 26, height: 26, cursor: 'pointer', fontSize: 14 }}>×</button>
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <label htmlFor="post-image-input" className="btn-ghost" style={{ padding: '6px 10px', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}>🖼️ Add image</label>
            </div>
            <button onClick={handlePost} disabled={posting} className="btn-gold" style={{ padding: '8px 16px', borderRadius: 8, fontSize: 14 }}>{posting ? 'Posting...' : 'Post'}</button>
          </div>
        </div>
      )}
